ent-BaseWeaponPistolSunrise = BasePistol
    .desc = A rooty tooty point and shooty.
    .suffix = Sunrise
ent-WeaponPistolVP78 = combat pistol VP-70
    .desc = { ent-BaseWeaponPistol.desc }
ent-WeaponPistolG22 = Glock-22
    .suffix = Lethal
    .desc = { ent-BaseWeaponPistol.desc }
ent-WeaponPistolG22Auto = Glock-22 auto
    .suffix = Auto
    .desc = { ent-BaseWeaponPistol.desc }
ent-WeaponPistolG22Metus = Glock-22 Metus
    .desc = Made to order for the security of the Metus planetary prison. Semi-automatic design. Uses .35 Auto caliber ammunition.
    .suffix = Sunrise
ent-WeaponPistolHIB88M4 = HI Browning-88 M4
    .desc = { ent-BaseWeaponPistol.desc }
ent-WeaponPistolACP14 = ACP-14
    .desc = { ent-BaseWeaponPistol.desc }
ent-WeaponRevolverMatebaNew = Mateba
    .desc = { ent-BaseWeaponRevolver.desc }
ent-WeaponPistolDeagle = Desert Eagle
    .desc = { ent-BaseWeaponPistol.desc }
ent-WeaponPistolDeagleMetus = Desert Eagle Metus
    .desc = Scuffs from old hands mark the handle, heavy and steady, like a verdict passed more than once without trial.
    .suffix = Sunrise
ent-WeaponRevolverSpearhead = Spearhead autorevolver
    .desc = A bulky revolver, occasionally carried by assault troops and officers in SWAT, as well as civilian law enforcement. Fires .45 Magnum rounds.
ent-WeaponRevolverSpearheadBlack = Spearhead autorevolver
    .desc = { ent-WeaponRevolverSpearhead.desc }
ent-WeaponPistolM1984 = D1984
    .desc = A compact sidearm developed under Nanotrasen’s internal order directive “N1984.” A personal weapon of control, discipline, and precise response. Issued to distinguished detectives for reliability during field investigations. Fires .35 Auto.
